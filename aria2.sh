...
# Description: Aria2
# Version: 2.0.8
# Author: white-hat-hacker 白帽珊蒂 
# Blog: https://hack.chat/
#=================================================
sh_ver="2.0.8"
...
check_root(){
    [[ $EUID != 0 ]] && echo -e "${Error} 你的帳號沒有root選縣，無法繼續操作 ${Green_background_prefix}sudo su${Font_color_suffix} 命令获取临时ROOT权限（执行后可能会提示输入当前账号的密码）。" && exit 1
}
#檢察系統
check_sys(){
...
check_installed_status(){
    [[ ! -e ${aria2c} ]] && echo -e "${Error} Aria2 未安裝，請檢查 !" && exit 1
    [[ ! -e ${aria2_conf} ]] && echo -e "${Error} Aria2 配置文件不存在，請檢查 !" && [[ $1 != "un" ]] && exit 1
}
check_crontab_installed_status(){
    if [[ ! -e ${Crontab_file} ]]; then
        echo -e "${Error} Crontab 開始安裝..."
        ...
        if [[ ! -e ${Crontab_file} ]]; then
            echo -e "${Error} Crontab 安裝失敗，請重新操作！" && exit 1
        else
            echo -e "${Info} Crontab Aria2-pro安裝成功！"
        fi
    fi
}
check_pid(){
...
check_new_ver(){
    echo -e "${Info} 請輸入 Aria2 版本，格式如：[ 1.35.0 ]，探查地址：[ https://github.com/q3aql/aria2-static-builds/releases ]"
    read -e -p "默認Enter按鍵將會自動安裝最新版本:" aria2_new_ver
    if [[ -z ${aria2_new_ver} ]]; then
        ...
        if [[ -z ${aria2_new_ver} ]]; then
            echo -e "${Error} Aria2 最新版本探查失敗，請手動選擇[ https://github.com/q3aql/aria2-static-builds/releases ]"
            read -e -p "請輸入版本 [ 格式如 1.35.0 ] :" aria2_new_ver
            [[ -z "${aria2_new_ver}" ]] && echo "取消..." && exit 1
        else
            echo -e "${Info} 檢查到 Aria2 最新版本為 [ ${aria2_new_ver} ]"
        fi
    else
        echo -e "${Info} 開始準備下载 Aria2 版本為 [ ${aria2_new_ver} ]"
    fi
}
check_ver_comparison(){
    aria2_now_ver=$(${aria2c} -v|head -n 1|awk '{print $3}')
    [[ -z ${aria2_now_ver} ]] && echo -e "${Error} Aria2 探查版本失敗 !" && exit 1
    if [[ "${aria2_now_ver}" != "${aria2_new_ver}" ]]; then
        echo -e "${Info} 發現 Aria2 已有新版本 [ ${aria2_new_ver} ](目前版本：${aria2_now_ver})"
        read -e -p "是否更新(更新會中斷下載任務，请注意) ? [Y/n] :" yn
        [[ -z "${yn}" ]] && yn="y"
        if [[ $yn == [Yy] ]]; then
            check_pid
            [[ ! -z $PID ]] && kill -9 ${PID}
        ...
        echo -e "${Info} 你目前 Aria2 已是最新版本 [ ${aria2_new_ver} ]" && exit 1
    fi
}
Download_aria2(){
    update_dl=$1
    ...
    [[ ! -s "${Aria2_Name}.tar.bz2" ]] && echo -e "${Error} Aria2 壓縮檔案下载失败 !" && exit 1
    tar jxvf "${Aria2_Name}.tar.bz2"
    [[ ! -e "/usr/local/${Aria2_Name}" ]] && echo -e "${Error} Aria2 解壓縮失敗 !" && rm -rf "${Aria2_Name}.tar.bz2" && exit 1
    [[ ${update_dl} = "update" ]] && rm -rf "${Folder}"
    mv "/usr/local/${Aria2_Name}" "${Folder}"
    [[ ! -e "${Folder}" ]] && echo -e "${Error} Aria2 文件夾重新命名失敗 !" && rm -rf "${Aria2_Name}.tar.bz2" && rm -rf "/usr/local/${Aria2_Name}" && exit 1
    rm -rf "${Aria2_Name}.tar.bz2"
    cd "${Folder}"
    make install
    [[ ! -e ${aria2c} ]] && echo -e "${Error} Aria2 主要程式安裝失敗！" && rm -rf "${Folder}" && exit 1
    chmod +x aria2c
    echo -e "${Info} Aria2 主要程式安装完成！"
}
Download_aria2_conf(){
    ...
    echo -e "${Info} Aria2－pro  最強版本下載完成！"
}
Service_aria2(){
    if [[ ${release} = "centos" ]]; then
...
Install_aria2(){
    ...
    check_sys
    echo -e "${Info} 開始安裝/配置 交互程序..."
    Installation_dependency
    echo -e "${Info} 開始下載/安装 主要程式..."
    check_new_ver
    Download_aria2
    echo -e "${Info} 開始下載/安装 Aria2－pro 最強配置..."
    Download_aria2_conf
    echo -e "${Info} 開始下載/安装 服務腳本(init)..."
    Service_aria2
    Read_config
    aria2_RPC_port=${aria2_port}
    echo -e "${Info} 開始設定 iptables防火牆..."
    Set_iptables
    echo -e "${Info} 開始增加 iptables防火牆參數..."
    Add_iptables
    echo -e "${Info}    保存 iptables防火墙參數..."
    Save_iptables
    echo -e "${Info} 開始建立 下載目錄..."
    mkdir -p /root/Download
    echo -e "${Info} 所有流程 安裝完成，啟動中..."
    Start_aria2
}
Start_aria2(){
    check_installed_status
    check_pid
    [[ ! -z ${PID} ]] && echo -e "${Error} Aria2 執行中，請檢查 !" && exit 1
    /etc/init.d/aria2 start
}
Stop_aria2(){
    check_installed_status
    check_pid
    [[ -z ${PID} ]] && echo -e "${Error} Aria2 未執行，請檢查 !" && exit 1
...
Set_aria2(){
 ...
 ${Green_font_prefix}3.${Font_color_suffix}  修改 Aria2 文件下載位置
 ${Green_font_prefix}4.${Font_color_suffix}  修改 Aria2 密碼+端口+文件下载位置
 ${Green_font_prefix}5.${Font_color_suffix}  手動 修改環境參數配置
 ————————————
 ${Green_font_prefix}0.${Font_color_suffix}  還原初始設定/更新 Aria2－pro 完美配置" && echo
    read -e -p "(默认: 取消):" aria2_modify
    [[ -z "${aria2_modify}" ]] && echo "取消完成..." && exit 1
    if [[ ${aria2_modify} == "1" ]]; then
        ...
        echo -e "${Error} 請輸入正確的數字(0-5)" && exit 1
    fi
}
Set_aria2_RPC_passwd(){
    read_123=$1
    ...
    if [[ -z "${aria2_passwd}" ]]; then
        aria2_passwd_1="空(沒有檢測到配置，請注意密碼中不要輸入此特殊符號”“)"
    else
        aria2_passwd_1=${aria2_passwd}
    fi
    echo -e "請輸入要配置的 Aria2 RPC密碼(舊密碼：${Green_font_prefix}${aria2_passwd_1}${Font_color_suffix})"
    read -e -p "(默認密碼: 隨機產生 密碼請不要有 = 和井号 #):" aria2_RPC_passwd
    echo
    ...
    if [[ "${aria2_passwd}" != "${aria2_RPC_passwd}" ]]; then
        if [[ -z "${aria2_passwd}" ]]; then
            ...
            if [[ $? -eq 0 ]];then
                echo -e "${Info} 密碼修改成功！新密碼為：${Green_font_prefix}${aria2_RPC_passwd}${Font_color_suffix}(因為找不到舊的環境參數，所以自動加入配置文件底部)"
                if [[ ${read_123} != "1" ]]; then
                    Restart_aria2
                fi
            else 
        ...
        echo -e "${Error} 新舊密碼請不要一致這樣非常危險，強制取消..."
    fi
}
Set_aria2_RPC_port(){
    read_123=$1
...
echo && echo -e " Aria2－Pro 快速部署管理腳本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  -- \033[1;35mhack.chat\033[0m --
  
 ${Green_font_prefix} 0.${Font_color_suffix} 升級脚本
————————————
 ${Green_font_prefix} 1.${Font_color_suffix} 安裝 Aria2-Pro
 ${Green_font_prefix} 2.${Font_color_suffix} 更新 Aria2-Pro
 ${Green_font_prefix} 3.${Font_color_suffix} 移除 Aria2-Pro
————————————
 ${Green_font_prefix} 4.${Font_color_suffix} 執行 Aria2-Pro
 ${Green_font_prefix} 5.${Font_color_suffix} 停止 Aria2-Pro
 ${Green_font_prefix} 6.${Font_color_suffix} 重啟 Aria2-Pro
————————————
 ${Green_font_prefix} 7.${Font_color_suffix} 修改 配置
 ${Green_font_prefix} 8.${Font_color_suffix} 查看 配置
 ${Green_font_prefix} 9.${Font_color_suffix} 查看 日誌
 ${Green_font_prefix}10.${Font_color_suffix} 清空 日誌
————————————
 ${Green_font_prefix}11.${Font_color_suffix} 手動更新 BT-Tracker
 ${Green_font_prefix}12.${Font_color_suffix} 自動更新 BT-Tracker
————————————" && echo
if [[ -e ${aria2c} ]]; then
    check_pid
    if [[ ! -z "${PID}" ]]; then
